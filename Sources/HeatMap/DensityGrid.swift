...
struct DensityGrid: Sendable {
    ...
    ///
    /// This method checks for `Task` cancellation periodically during the
    /// Gaussian kernel accumulation phase. When called outside of a `Task`
    /// context, the cancellation checks are a no-op.
    ///
    /// - Parameters:
    ...
    /// - Throws: `CancellationError` if the current task is cancelled during
    ///   computation.
